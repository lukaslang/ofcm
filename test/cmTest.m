...
assertEqual(size(cmc), [2*size(X, 1), 1]);
assertAlmostEqual(cmc, zeros(2*size(X, 1), 1));
...
v = bsxfun(@times, (bfc1')*cmc, d1) + bsxfun(@times, (bfc2')*cmc, d2);
clear bfc1;
clear bfc2;
