...
function v = surfdivmem(Ns, cs, xi, k, h, X, mem)
%SURFDIVMEM Evaluates the surface divergence of vectorial basis functions.
%   
%   v = SURFDIVMEM(Ns, cs, xi, k, h, X, mem) takes a surface description 
%   Ns, cs, evaluation points xi, a degree k, a parameter h, and a matrix X
%   of locations of the basis functions, and returns the surface divergence
%   of the vector field spanned by the basis functions. mem > 0 specifies 
%   the memory used for matrix multiplication.
%
...
[bfc1, bfc2] = vbasiscompmem(k, h, X, Y, mem);

% Evaluate partial derivatives of basis functions.
[bfcd{1, 1}, bfcd{1, 2}, bfcd{2, 1}, bfcd{2, 2}] = vbasiscompderivmem(k, h, X, Y, mem);
