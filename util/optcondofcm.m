...
%   
%   The linear systems returned are
%
%   (Aof + alpha * D + beta * E) * x = bof
%
%   and
%
%   (Acm + alpha * D + beta * E + gamma * G) * x = bcm
%
%   Aof, Acm, D, E, and G are matrices of size [2*m, 2*m].
%   bof, bcm are the right hand sides and are of length 2*m.
...
ipof = sparse(bsxfun(@times, bfc{1}, dot(d1, gradf, 2)') + bsxfun(@times, bfc{2}, dot(d2, gradf, 2)'));
ipcm = sparse(bsxfun(@times, bfc{1}, dot(d1, gradf, 2)') + bsxfun(@times, bfc{2}, dot(d2, gradf, 2)') + bsxfun(@times, sdiv, f'));
clear d1;
clear d2;
...
Aof = ipof*spdiags(intf, 0, n, n)*(ipof');
Acm = ipcm*spdiags(intf, 0, n, n)*(ipcm');
...
bof = -ipof*spdiags(intf, 0, n, n)*dtdf;
bcm = -ipcm*spdiags(intf, 0, n, n)*(dtdf - ipvtan - f .* K .* V);
